...
def preProc(filename, num_feat):
	...
	#print("Length of token vector is ", str(len(token_vector)))
...
def accCalc(trainX, trainY, testX, testY, weight, layer, threshold):
	...
	for i in range(0, len(testX)):
		...
		if(out==testY[i]):
			if(testY[i]==1):
				...
				true_ham = true_ham + 1
			#print("right: ", str(output), str(testY[i]))
		else:
			if(testY[i]==1):
				false_ham = false_ham + 1
			else:
				false_spam = false_spam + 1
			#print("wrong: ", str(output), str(testY[i]))
	...
	plt.title("Threshold estimation in sigmoid")
	plt.xlabel("Number of training examples")
	plt.ylabel("Output of neural network")
	plt.legend(['Spam', 'Ham'])
	plt.show()

	fig.savefig('sigmoid_threshold.png')
	plt.close(fig)
...
def learn(dataset, layer, alpha, threshold):
	...
	for i in range(0, num_feat+1):
		for j in range(0, layer[0]):
			...
			#number = number/100
			#temp.append(number)
			#temp.append(rand.uniform(-1.0, 1.0))
	...
	while(inerr>0.02):
		...
		delta[num_layers-1][0] = 2*(layer_output[num_layers-1][0] - trainY[n])*layer_output[num_layers-1][0]*(1 - layer_output[num_layers-1][0])
	...
	plt.close(fig)
	print("")
	print("Figure saved as sigmoid.png")
	print("")
...
def main():
	...
	layer = [100, 50, 1]
	weight = learn(dataset, layer, 0.1, 0.3)
	accCalc(dataset[0], dataset[1], dataset[2], dataset[3], weight, layer, 0.3)
	print("")
