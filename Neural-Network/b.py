...
def classify(weight, input_vector, layer, threshold):
	...
	layer_output[0] = 1/(1+np.exp(np.dot(-1, layer_input[0])))
	layer_output[0][0] = 1
	...
	for l in range(1, num_layers-1):
		...
		layer_output[l] = 1/(1+np.exp(np.dot(-1, layer_input[l])))
		layer_output[l][0] = 1

	layer_input[num_layers-1] = np.dot([layer_output[num_layers-2]], weight[num_layers-1])[0]

	summation = 0.0
	for i in range(0,layer[num_layers-1]):
		summation = summation + np.exp(layer_input[num_layers-1][i])

	layer_output[num_layers-1] = np.multiply(1/summation, np.exp(layer_input[num_layers-1]))	

	if(layer_output[num_layers-1][0]>layer_output[num_layers-1][1]):
		return 1 - layer_output[num_layers-1][0]
	else:
		return layer_output[num_layers-1][1]
...
def accCalc(trainX, trainY, testX, testY, weight, layer, threshold):
	...
	plt.title("Threshold estimation in softmax")
	plt.xlabel("Number of training examples")
	plt.ylabel("Output of neural network")
	plt.legend(['Spam', 'Ham'])
	plt.show()

	fig.savefig('softmax_threshold.png')
	plt.close(fig)
...
def learn(dataset, layer, alpha, threshold):
	...
	for i in range(0, num_feat+1):
		for j in range(0, layer[0]):
			...
			#number = -1.0 * float(number)
			#number = number/100
			#temp.append(number)
			#temp.append(rand.uniform(-1.0, 0.0)/10)
	...
	while(inerr>0.02):
		...
		#print(np.exp(layer_input[num_layers-1]))
		#print(summation)
		...
		delta[num_layers-1][0] = (layer_output[num_layers-1][0] + trainY[n] - 1)*layer_output[num_layers-1][0]*(1 - layer_output[num_layers-1][0])
		delta[num_layers-1][1] = (layer_output[num_layers-1][1] - trainY[n])*layer_output[num_layers-1][1]*(1 - layer_output[num_layers-1][1])

		l = num_layers-2
		while(l>=0):
			delta[l] = np.multiply(np.dot([delta[l+1]], np.transpose(weight[l+1]))[0], np.multiply(layer_output[l], np.subtract(1, layer_output[l])))
			l = l - 1
	...
	plt.close(fig)
	print("")
	print("Figure saved as softmax.png")
	print("")
...
def main():
	...
	layer = [100, 50, 2]
	weight = learn(dataset, layer, 0.1, 0.4)
	accCalc(dataset[0], dataset[1], dataset[2], dataset[3], weight, layer, 0.4)
	print("")
